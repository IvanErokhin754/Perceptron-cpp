...
public:    
    ...
    int Predict(const std::vector<double>& input) const {
        std::vector<double> x_norm = Transform(input);
    ...
    void TrainBatch(const Matrix<double>& X, const std::vector<double>& y) {
        size_t m = X.GetRows(); // размер батча
        if (m == 0) return;

        std::vector<double> z = X.MultiplyByVector(weights);
        ...
        std::vector<double> weights_grad = X.TransposeMultiplyByVector(diff);
    ...
    double ComputeLoss(const std::vector<TrainingSample>& X) const {
        double loss = 0.0;
        size_t m = X.size();
        for (size_t i = 0; i < m; i++) {
    ...
    void Train(std::vector<TrainingSample> X, int epochs, size_t batch_size = 32) {
        if (epochs <= 0)
        ...
        for (int epoch = 0; epoch < epochs; epoch++) {
            ...
            for (size_t i = 0; i < X.size(); i += batch_size) {
                ...
                std::vector<double> y_batch(current_batch_size);
    ...
    void Evaluate(const std::vector<TrainingSample>& test_data) const {
        int TP = 0;
        int TN = 0;
        int FP = 0;
        int FN = 0;
    ...
    void FitScaler(const std::vector<TrainingSample>& X) {
